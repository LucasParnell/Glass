...
Base::Result _cdecl
TextureLoader::teCreateTexture(Components::Texture
&texture,
...
texture.
ID = iTexture;
mountedTextures[mountpoint+filename] = texture.
ID;
...
{
texture.
ID = mountedTextures[mountpoint + filename];
}

texture.
loaded = true;
return
Base::Result::STATUS_OK;
}