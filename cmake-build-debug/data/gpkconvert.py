...
class GPK:
    ...
    def _openFile(self):
        ...
        if self.filemode == FileMode.WRITE: #Read file into buffer first
            if os.path.isfile(self.target) and os.stat(self.target).st_size > 0:
                ...
                self.buffer = b"GPAK" + bytes(baseHeader.isCompressed) + baseHeader.name + padName + baseHeader.entries.to_bytes(4, byteorder='little')
                LenFileTable = 0x1000
    ...
    def addFileEntry(self, mountpoint, filename, isCompressed):
        ...
        binFileHeader = fileHeader.mountPoint + padMountPoint + fileHeader.name + padName + bytes(fileHeader.isCompressed) + fileHeader.size.to_bytes(4, byteorder='little')
